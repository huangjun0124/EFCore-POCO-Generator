...
SELECT  SERVERPROPERTY('Edition') AS Edition,
                ...
                while (rdr.Read())
                {
                    ...
                    if (lastTable != tableName || table == null)
                    {
                        ...
                        if (table == null)
                        {
                            ...
                            if (CommonParams.ReservedKeywords.Contains(table.NameHumanCase) ||
                ...
                while (rdr.Read())
                {
                    ...
                    if (rdr["DATA_TYPE"] != null && rdr["DATA_TYPE"] != DBNull.Value)
                    {
                        ...
                        if (!string.IsNullOrEmpty(clean))
                        {
                            ...
                            if (CommonParams.ReservedKeywords.Contains(param.NameHumanCase))
        ...
        public override void ProcessForeignKeys(List<ForeignKey> fkList, Tables tables, bool checkForFkNameClashes)
        {
            ...
            foreach (var constraint in constraints)
            {
                ...
                var relationship = Processer.CalcRelationship(pkTable, fkTable, fkCols.Select(c => c.col).ToList(), pkCols);
        ...
        private static Column CreateColumn(IDataRecord rdr, Regex rxClean, Table table, Regex columnFilterExclude)
        {
            ...
            table.HasNullableColumns = Processer.IsNullable(col);
